...
export function convertOHLCData(data: OHLCData[]) {
	return data
		.map((d) => {
			// CoinGecko returns timestamp in milliseconds. Lightweight charts expects seconds.
			// If it's larger than a reasonable Unix timestamp in seconds (e.g., year 3000), it's likely ms.
			const timeInSeconds = d[0] > 32503680000 ? Math.floor(d[0] / 1000) : d[0];
			return {
